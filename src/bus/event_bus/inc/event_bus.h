 ...
 * @details 发布-订阅模式，线程安全，单例设计，无全局变量
 * @author Luo
 * @date 2026-05-31
 */
...
// 不透明句柄（仅内部使用，外部无需操作）
// ==========================================================================
typedef struct event_bus_item_t* event_bus_subscription_handle_t;

// ==========================================================================
// 事件优先级（预留扩展，当前默认使用普通优先级）
...
typedef struct {
    ...
    const char *name;             // 总线唯一名称（多实例标识）
} event_bus_config_t;

// ==========================================================================
// 事件总线核心对外接口 - 全部使用字符串名称传参
// ==========================================================================

/**
 * @brief 初始化事件总线
 * @param config 总线配置参数（包含名称）
 * @return 0成功，负数失败
 */
