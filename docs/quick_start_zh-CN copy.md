...
### 2.4 批量拷贝文件至NFS共享目录
将**可执行程序、AI模型、所有第三方动态库**统一拷贝到PC端NFS共享目录 `~/nfs/run_on_board_rk3562`，开发板通过NFS挂载访问：
```bash
mkdir -p ~/nfs/run_on_board_rk3562/{mnn,libjpeg,openh264,libyuv,drv}
# 1. 拷贝主程序
cp output/vision_ai_app ~/nfs/run_on_board_rk3562/

cp -rf boards/aarch64/drv/led_drv/*.{ko,dtb}  ~/nfs/run_on_board_rk3562/drv
# 2. 拷贝MNN AI模型文件
cp third_lib/aarch64/face_detector/model/version-RFB/RFB-320-quant-KL-5792.mnn ~/nfs/run_on_board_rk3562/

# 3. 拷贝第三方动态库（按目录分类）
cp -rf third_lib/aarch64/face_detector/mnn/lib/libMNN.so ~/nfs/run_on_board_rk3562/mnn
...
# 将NFS内完整项目文件同步至板端固定目录 如:/root/run_on_board_rk3562
mkdir run_on_board_rk3562
cp -rp /mnt/run_on_board_rk3562 /root/

# cp -rp /mnt/run_on_board_rk3562/vision_ai_app /root/run_on_board_rk3562/
...
rmmod /root/run_on_board_rk3562/drv/board_A_led 2>/dev/null
rmmod /root/run_on_board_rk3562/drv/chip_demo_gpio 2>/dev/null
rmmod /root/run_on_board_rk3562/drv/leddrv 2>/dev/null

# 2. 按顺序加载驱动（固定加载顺序，不可颠倒）
insmod /root/run_on_board_rk3562/drv/leddrv.ko 2>/dev/null
insmod /root/run_on_board_rk3562/drv/chip_demo_gpio.ko 2>/dev/null
insmod /root/run_on_board_rk3562/drv/board_A_led.ko 2>/dev/null
...
# 4. 驱动功能测试（可选）
/root/run_on_board_rk3562/drv/ledtest /dev/100ask_led0 on   # LED点亮
/root/run_on_board_rk3562/drv/ledtest /dev/100ask_led0 off  # LED熄灭
```
